...
for i = 3:length(dir) % runing through the folder
    ...
    disp(['current inverse for sum compression/error ' num2str( 1/(Errors(k)+Comp_rates(k)))])
...
disp(['Average inverse for sum compression/error ' num2str( 1/(Result(1)+Result(2)))])
...
% with net_decding optimization

% k=6, z=4 ; 3 bit qanitization with net_decding optimization
% Average quadratic error: 0.0053009
% Average compression rate: 0.0089041
% Average inverse for sum compression/error 70.3979

% same
% Time for everything: 429.8119
% Average quadratic error: 0.0054458
% Average compression rate: 0.0089041
% Average inverse for sum compression/error 69.687